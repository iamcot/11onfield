...
import { PlayerListItem, PlayersFilters, getLevelDisplayName, getPositionDisplayName, getPreferredFootDisplayName } from "@/types/player";
...
function PlayersContent() {
  ...
  useEffect(() => {
    ...
    if (filters.positions && filters.positions.length > 0) params.set("positions", filters.positions.join(","));
  ...
  useEffect(() => {
    if (!authLoading && isInitialized) {
      const fetchPlayers = async () => {
        try {
          setIsLoading(true);
          const response = await playerService.getPlayers(pagination.page, pagination.pageSize, filters);
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col md:flex-row">
      ...
      <main className="flex-1 overflow-auto pb-16 md:pb-0">
        <div className="max-w-7xl mx-auto px-4 py-6">
          {/* Page Header */}
          <div className="mb-6">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Danh sách cầu thủ</h1>
            <p className="text-gray-600 mt-1">Tìm kiếm và khám phá cầu thủ trên hệ thống</p>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-4 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              ...
              <select
                ...
                onChange={(e) => handleFilterChange("positions", e.target.value ? [e.target.value] : [])}
              ...
              <select
                ...
                onChange={(e) => handleFilterChange("preferredFoot", e.target.value || null)}
                className="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 text-sm"
              >
            ...
            {(searchInput || filters.positions?.length || filters.level || filters.preferredFoot || filters.sortBy) && (
              <div className="mt-4 flex justify-end">
                <button
                  onClick={handleClearFilters}
                  className="px-4 py-2 text-sm text-red-600 hover:text-red-700 hover:bg-red-50 rounded-md transition flex items-center gap-2"
                >
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            {isLoading ? (
              ...
              <div className="p-8 text-center text-gray-500">Không tìm thấy cầu thủ nào</div>
            ) : (
              <>
                {/* Desktop Table */}
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {players.map((player) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-sm text-gray-600">
                            {player.preferredFoot ? getPreferredFootDisplayName(player.preferredFoot) : "N/A"}
                          </td>
                          <td className="px-4 py-3 text-sm text-gray-600">
                        ...
                        <div>
                          ...
                          <span className="font-medium text-gray-900">
                            {player.positions.length > 0
                              ? player.positions.map(getPositionDisplayName).join(", ")
                              : "N/A"}
                          </span>
                        </div>
                        <div>
